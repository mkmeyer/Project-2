...
standings_query <- function(season, team){
  ...
  standings_data$conference_name <- as.numeric(standings_data$conference$name)
...
```{r}
getstandingsData <- function(season, team){
...
standings <- renderTable({
    ...
    write.csv(standingstableData, "teamData.csv")
    head(standingstableData)
  })

standingstableData <- getstandingsData(2020, 1)
write.csv(standingstableData, "teamData.csv")
head(standingstableData)
...
g <- ggplot(standingsData_long, aes(x = Metric, y = Count)) + geom_bar(stat = "identity")
g
```
